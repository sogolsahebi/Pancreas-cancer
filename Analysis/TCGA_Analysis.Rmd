...
# Set BiocParallel to use a single core to see if the problem persists
...
write.table(fgseaResTidy, "~/BHK lab/Pancreas-cancer/output data/TCGA_fgseaRes_HRank.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
# For significant pathways # 8 pathways.
...
write.table(fgseaResTidy_kegg, "~/BHK lab/Pancreas-cancer/output data/TCGA_fgseaRes_kegMEDRank.txt", sep = "\t", row.names = FALSE, quote = FALSE)
 

```
no significant pathway is found based on 'padj'

```{r KEGG plot ,fig.width= 12 , fig.height= 9 }

#1.1 no significant pathway is detected based on 'padj'
...
#1.2 check pathway based on 'pval'
fgseaResTidy_kegg$sigpval <- ifelse(fgseaResTidy_kegg$pval <= 0.05, "significant", "non-significant")  

ggplot(fgseaResTidy_kegg[fgseaResTidy_kegg$pval < 0.05, ], aes(reorder(pathway, NES), NES, fill = sigpval)) +
  geom_col() +
  scale_fill_manual(values = cols) +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
# Plot significant pathways as points with a gradient color based on p-value and size based on a 'size' variable.
ggplot(fgseaResTidy_kegg[fgseaResTidy_kegg$pval < 0.05, ], aes(y=reorder(pathway, size),x= NES)) + geom_point(aes(color=pval,size=size)) + 
  scale_color_gradientn(colours = c("red","blue")) + 
  labs(x='Normalized Enrichment Score', y=NULL ) + 
  theme( axis.title = element_text(face='bold'), axis.text = element_text (face='bold') )
...
# Save fgseaRes_keg as txt file.
write.table(fgseaResTidy_kegg, "~/BHK lab/Pancreas-cancer/output data/TCGA_fgseaRes_keggLEGRank.txt", sep = "\t", row.names = FALSE, quote = FALSE)

```

no significant pathway based on 'padj' 
...
#1.1 no significant pathway based on 'padj'
...
#1.2 check  significant pathway based on 'pval'
